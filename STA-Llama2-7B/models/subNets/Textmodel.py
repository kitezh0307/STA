...
class Language_model (nn.Module):
    ...
    def forward(self, fusion_embedding, labels):
        ...
        opt_tokens, atts_bos, atts_fusion, labels, labels_atts = self.input_processing(fusion_embedding, labels, mode = 'train')
        attention_mask = torch.cat([atts_bos, atts_fusion, labels_atts], dim=1)


        with torch.cuda.amp.autocast():
            output = self.model(inputs_embeds = opt_tokens, return_dict=True, labels = labels)
